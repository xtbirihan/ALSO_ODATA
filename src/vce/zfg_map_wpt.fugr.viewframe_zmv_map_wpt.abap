*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZMV_MAP_WPT
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZMV_MAP_WPT         .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZMV_MAP_WPT'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZMV_MAP_WPT_EXTRACT
TOTAL = ZMV_MAP_WPT_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZMV_MAP_WPT'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZMV_MAP_WPT-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZMV_MAP_WPT_EXTRACT
TOTAL = ZMV_MAP_WPT_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZMV_MAP_WPT-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZMV_MAP_WPT'
...
TOTAL = ZMV_MAP_WPT_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZMV_MAP_WPT-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZMV_MAP_WPT-UPD_FLAG NE SPACE.
